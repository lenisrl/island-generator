...
export class IslandGenerator {
  ...
  private applyBiomesVoronoi(grid: Tile[][], params: GenerationParams) {
    ...
    // Biome pool (Heavily weighted towards JUNGLE as requested)
    const availableBiomes = [
      BiomeType.JUNGLE, BiomeType.JUNGLE, BiomeType.JUNGLE, BiomeType.JUNGLE, // 4x Jungle
      BiomeType.SAVANNA,
      BiomeType.MOUNTAIN,
    ...
    for (let y = 0; y < params.height; y++) {
      for (let x = 0; x < params.width; x++) {
        if (grid[y][x].elevation === 1) {
          ...
          let chosenBiome = BiomeType.JUNGLE;
